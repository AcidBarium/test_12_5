...
} yuanshi[200005], arr[200006], st1[200006], st2[200006];
...
inline ld getS(const point &a, const point &b, const point &c)
{
    return abs(crossMul(a, b, a, c)) / 2.0;
}
...
inline void solve()
{
    ...
    _rep(i, 1, p1)
    {
        ...
        while (left < right)
        {
            if (crossMul(st1[i], st1[i + 1], st2[mid - 1], st2[mid]) <= 0)
            {
                anstemp = mid;
                left = mid + 1;
            }
        ...
        sMin = min(sMin, getS(st1[i], st1[i + 1], st2[anstemp]));
    }
    ...
    cout << 2 * (long long)(sSum - sMin) << endl;
}
