...
ld dis(const point &a, const point &b, const point &c)
{
    ld s = abs(crossMul(a, b, a, c));
    ld l = dis(b, c);
    return s / l;
}
...
int main()
{
    ...
    while (cin >> n)
    {
        ...
        _rep(i, 2, n)
        {
            while (p > 1 && crossMul(st[p - 1], st[p], st[p - 1], arr[i]) <= 0)
                p--;
            st[++p] = arr[i];
        }
        ...
        _rep(i, 1, p)
        {
            st[i + p] = st[i + p * 2] = st[i + p * 3] = st[i + p * 4] = st[i + p * 5] = st[i + p * 6] = st[i + p * 7] = st[i];
        }
        ...
        _rep(i, 1, p)
        {
            while (dis(st[cur], st[i], st[i + 1]) <= dis(st[cur + 1], st[i], st[i + 1]))
            {
                cur = (cur + 1);
            }
            ans = min(ans, dis(st[cur], st[i], st[i + 1]));
        }
        cout << "Case " << testCnt << ": ";
